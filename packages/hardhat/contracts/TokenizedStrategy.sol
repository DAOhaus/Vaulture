...
contract TokenizedStrategy {
	...
	event AllocatedToExternalAccount(address indexed recipient, uint256 amount);
	event ReturnedFromExternalAccount(address indexed sender, uint256 amount);
	...
	struct StrategyData {
        ...
        uint256 allocatedToExternal;  // Assets allocated to external accounts
	...
	function report(
		uint amount
	) external nonReentrant onlyKeepers returns (uint256 profit, uint256 loss) {
		StrategyData storage S = _strategyStorage();
		require(amount > 0, "Amount must be greater than zero");
		require(
			amount <= S.allocatedToExternal,
			"Cannot return more than allocated externally"
		);
		...
		// Update the tracking of external allocations within the strategy data
	...
	/**
	 * @dev On contract creation we set `asset` for this contract to address(1) ??? why? updating to 0
	...
	/*//////////////////////////////////////////////////////////////
                            EXTERNAL
    //////////////////////////////////////////////////////////////*/
	// This is the only function we actually need to define.
	// freefunds doesn't apply because there is no way to free off-chain funds
	// because we have to alter "report" to include an amount we are just altering that contract
	function deployFunds(uint256 amount, address recipient) internal {
		require(recipient != address(0), "Invalid recipient address");
		require(amount > 0, "Amount must be greater than zero");
		require(
			amount <= _totalAssets(_strategyStorage()),
			"Insufficient assets in the strategy"
		);
